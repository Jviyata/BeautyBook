...
import { HAIR_TECHS } from '../techData'

export default function HairTechsPage() {
  return (
    <div className="min-h-screen bg-[#f7f7f7]">
      ...
      <main className="page-shell space-y-4">
        <section className="rounded-3xl border border-[#e8e8e8] bg-white p-6 sm:p-8">
          ...
          <h1 className="mt-2 font-display text-3xl text-[#1f1f1f]">Hair Techs</h1>
          <p className="mt-2 text-sm text-[#666]">{HAIR_TECHS.length} generated profiles</p>
        </section>

        <section className="rounded-3xl border border-[#e8e8e8] bg-white p-5">
          <ol className="grid gap-2 sm:grid-cols-2">
            {HAIR_TECHS.map((tech, index) => (
